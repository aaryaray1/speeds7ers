...
# Define a pickup scale to reduce the area where the power-up is collected (without reducing the sprite size)
POWERUP_PICKUP_SCALE = 0.5  # e.g., 50% of the drawn sprite area
...
class DQNAgent:
    ...
    def optimize_model(self, replay_buffer, batch_size):
        ...
        batch_action = torch.tensor([ACTIONS.index(a) for a in batch_action], dtype=torch.int64, device=self.device).unsqueeze(1)
...
def evaluate_move(pos, direction, player_pos, player_dir, player_trail, ai_trail, active_effects, depth=3):
    if depth == 0:
        return 0
    ...
    # Enforce a 3-block gap from the AI's trail (excluding the most recent cell)
    for tail_pos in list(ai_trail)[1:]:
        if abs(next_x - tail_pos[0]) + abs(next_y - tail_pos[1]) < 3:
            return -1000
    ...
    distance_bonus = (prev_distance - distance_to_player) * 2.0
    ...
    for next_dir in get_available_actions((direction[0], direction[1])):
        ...
        follow_up_score = evaluate_move((next_x, next_y), next_dir, player_pos, player_dir,
                                        player_trail, temp_ai_trail, active_effects, depth - 1)
    ...
    adjacent_count = 0
    for dx, dy in ACTIONS:
        neighbor = (next_x + dx, next_y + dy)
        if neighbor in list(ai_trail)[1:]:
            adjacent_count += 1
    if adjacent_count > 1:
        score -= adjacent_count * 500
...
def display_message(screen, message):
    ...
    while True:
        restart_clicked = create_button(screen, "Restart", (WIDTH // 2 - 300, HEIGHT // 2), 200, 50, (0, 128, 0), (0, 200, 0))
        menu_clicked = create_button(screen, "Main Menu", (WIDTH // 2 - 100, HEIGHT // 2), 200, 50, (0, 0, 128), (0, 0, 200))
...
def menu_screen():
    ...
    while True:
        ...
        start_clicked = create_button(screen, "Start Game", (WIDTH // 2 - 200, button_y), 200, 50, (0, 128, 0), (0, 200, 0))
        quit_clicked = create_button(screen, "Quit Game", (WIDTH // 2 + 50, button_y), 200, 50, (128, 0, 0), (200, 0, 0))
        pygame.display.flip()
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                save_model()
...
def game_loop(player_color, ai_color):
    ...
    powerup_types = ["double_speed", "invisibility", "wall_phase", "extra_life", "ai_confusion", "teleport", "slow_time"]
    POWERUP_DURATIONS = {
        "double_speed": 150,
        "invisibility": 150,
        "wall_phase": 150,
    ...
    while running:
        ...
        if len(powerups) < max_powerups and random.random() < powerup_spawn_probability:
            ...
            if (pu_x, pu_y) not in player_trail and (pu_x, pu_y) not in ai_trail and (pu_x, pu_y) != player_pos[0] and (pu_x, pu_y) != ai_pos[0]:
            ...
            for pu in powerups[:]:
                ...
                if player_rect.colliderect(pu_rect):
                    ...
                    if pu_type in ["double_speed", "invisibility", "wall_phase", "ai_confusion", "slow_time"]:
                        ...
                        current_state = get_state(ai_pos, player_pos, ai_dir, player_trail, ai_trail, False)
                    powerups.remove(pu)
            if active_effects["wall_phase"] <= 0 and (new_player_pos[0] < 0 or new_player_pos[0] >= COLS or new_player_pos[1] < 0 or new_player_pos[1] >= ROWS):
                if extra_lives > 0:
                    extra_lives -= 1
                    player_pos = [(5, ROWS // 2)]
                    player_trail.clear()
        ...
        if not (active_effects["slow_time"] > 0 and ai_move_counter % 2 == 1):
            ...
            for action in available_actions:
                ...
                if (next_x < 0 or next_x >= COLS or next_y < 0 or next_y >= ROWS or
                        (next_x, next_y) in player_trail or (next_x, next_y) in ai_trail):
                    continue
                score = evaluate_move(ai_pos[0], action, player_pos, player_dir, player_trail, ai_trail, active_effects)
                for pos in ai_trail:
                ...
                distance_to_player = abs(next_x - player_pos[0][0]) + abs(next_y - player_pos[0][1])
                distance_bonus = (prev_distance - distance_to_player) * 1.0
                score += distance_bonus
                if action == ai_dir:
                    score += 10
        ...
        reward = 0.05 + 1.0 if new_distance < prev_distance else 0.05 - 0.5
        steps_survived += 1
        ...
        game_end = False
        if (player_pos[0][0] < 0 or player_pos[0][0] >= COLS or player_pos[0][1] < 0 or player_pos[0][1] >= ROWS):
            game_end = True
        ...
        elif (new_ai_pos[0] < 0 or new_ai_pos[0] >= COLS or new_ai_pos[1] < 0 or new_ai_pos[1] >= ROWS):
            game_end = True
            reward = -50
        ...
        if game_end:
            total_games += 1
            if total_games % 10 == 0:
                print(f"Games played: {total_games}, Current epsilon: {agent.epsilon:.4f}, Updates: {agent.update_count}")
            action = display_message(screen, game_over_message)
            if action == "restart":
                return "restart"
            elif action == "menu":
        ...
        controls_text = font.render("ESC: Main Menu | T: Toggle Training | Up/Down: Change Difficulty", True, (200, 200, 200))
